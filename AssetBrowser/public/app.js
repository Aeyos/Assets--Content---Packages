(() => {
  ...
  function rebuildFilterPills() {
    // Category/theme/pack pill lists reflect what's reachable given the
  ...
  function makeThumbImg(item) {
    ...
    img.loading = 'lazy';
    img.src = `/files/${item.thumb.split('/').map(encodeURIComponent).join('/')}?v=${Date.now()}`;
    img.alt = item.name;
    return img;
  }
  ...
  function buildCardActions(item, thumb) {
    ...
    bar.append(copyBtn, regenBtn, revealBtn);
    return bar;
  }
  ...
  function renderCard(item) {
    ...
    card.title = item.renderable ? 'Click to open 3D viewer' : 'Click to reveal in Explorer';
    ...
    if (item.thumb) {
      ...
      span.textContent = PLACEHOLDER_ICON[item.category] || '❔';
      thumb.appendChild(span);
    }
    thumb.appendChild(buildCardActions(item, thumb));
    ...
    meta.append(name, sub, formats);
    card.append(thumb, meta);

    card.addEventListener('click', () => {
      if (item.renderable) openViewer(item);
      else reveal(item);
    });
    return card;
  }
  ...
  async function openViewer(item) {
    ...
    // webassembly bindings). closeActiveViewer() above guarantees at most one
